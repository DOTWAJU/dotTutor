...
import { ArrowLeft, User, Sms, Lock, Eye, EyeSlash, TickCircle } from 'iconsax-react';
...
export function StudentRegisterPage({ onBack, onLogin, onSubmit }: StudentRegisterPageProps) {
    ...
    <div className="min-h-screen bg-white relative overflow-hidden">
      ...
      <div className="flex items-center justify-center min-h-screen px-4 sm:px-6 lg:px-8 py-20 relative z-10">
        <div className="w-full max-w-[1100px]">
          ...
          <div className="flex justify-center mb-10 sm:mb-12 lg:hidden">
            <Logo />
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-8 lg:gap-16 items-center">
            ...
            <div className="hidden lg:flex flex-col items-center justify-center">
              {/* Character Image with Logo Overlay */}
              <div className="relative w-full max-w-[450px] mx-auto">
                ...
                {/* Logo Centered in Image - Top Position */}
                <div className="absolute top-8 left-1/2 -translate-x-1/2 z-20">
                  <Logo />
                </div>
                
                {/* Decorative Circle Behind */}
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[85%] h-[85%] bg-gradient-to-br from-[#FF6636]/10 to-[#FF6636]/5 rounded-full blur-2xl -z-10" />
              </div>

              {/* Info Text */}
              <div className="mt-8 text-center max-w-md">
                <h2 className="text-[28px] font-bold mb-3" style={{ color: 'rgb(1, 27, 51)' }}>
                  Join 10,000+ Students
                </h2>
                <p className="text-[15px] text-[#52565c] leading-relaxed">
            ...
            {/* Right Side - Form */}
            <div className="w-full max-w-[520px] lg:max-w-[440px] mx-auto lg:mx-0">
                ...
                <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
                  ...
                  <div className="group">
                    <label 
                      ...
                      style={{ color: focusedField === 'fullName' ? '#FF6636' : 'rgb(1, 27, 51)' }}
                    >
                      Full Name
                    </label>
                    <div className="relative">
                      <div className="absolute left-3.5 sm:left-4 top-1/2 -translate-y-1/2 z-10 transition-transform duration-200 group-focus-within:scale-110">
                        <User 
                          ...
                          variant={getFieldStatus('fullName') === 'focused' ? 'Bold' : 'Linear'}
                        />
                      </div>
                      <input
                        id="fullName"
                        ...
                        className={`w-full h-12 sm:h-14 pl-11 sm:pl-12 pr-11 sm:pr-12 rounded-xl border text-[14px] sm:text-[15px] transition-all duration-300 outline-none bg-white/50 backdrop-blur-sm ${ 
                          getFieldStatus('fullName') === 'focused' ? 'border-[#FF6636] shadow-lg shadow-[#FF6636]/10 bg-white' :
                          getFieldStatus('fullName') === 'valid' ? 'border-green-400 bg-white' :
                          getFieldStatus('fullName') === 'invalid' ? 'border-red-400' :
                    ...
                    {touched.fullName && !validations.fullName && (
                      <p className="text-[11px] sm:text-xs text-red-500 mt-1.5 ml-1 animate-in slide-in-from-top-1 duration-200">Name must be at least 3 characters</p>
                    )}
                  </div>
                  ...
                  <div className="group">
                    ...
                    {touched.password && !validations.password && (
                      <p className="text-[11px] sm:text-xs text-red-500 mt-1.5 ml-1 animate-in slide-in-from-top-1 duration-200">Password must be at least 8 characters</p>
                    )}
                  </div>
                  ...
                  <div className="group">
                    ...
                    <div className="relative">
                      <div className="absolute left-3.5 sm:left-4 top-1/2 -translate-y-1/2 z-10 transition-transform duration-200 group-focus-within:scale-110">
                        <Lock 
                          className="w-[18px] h-[18px] sm:w-5 sm:h-5 text-[#FF6636] transition-transform duration-200"
                          variant={getFieldStatus('confirmPassword') === 'focused' ? 'Bold' : 'Linear'}
                        />
                      </div>
                      <input
                        id="confirmPassword"
                        ...
                        className={`w-full h-12 sm:h-14 pl-11 sm:pl-12 pr-11 sm:pr-12 rounded-xl border text-[14px] sm:text-[15px] transition-all duration-300 outline-none bg-white/50 backdrop-blur-sm ${ 
                          getFieldStatus('confirmPassword') === 'focused' ? 'border-[#FF6636] shadow-lg shadow-[#FF6636]/10 bg-white' :
                          getFieldStatus('confirmPassword') === 'valid' ? 'border-green-400 bg-white' :
                          getFieldStatus('confirmPassword') === 'invalid' ? 'border-red-400' :
                          'border-[#E5E7EB] hover:border-[#FF6636]/40 hover:bg-white'
                        }`}
                        style={{ color: 'rgb(1, 27, 51)' }}
                      />
                    ...
                    {touched.confirmPassword && !validations.confirmPassword && (
                      <p className="text-[11px] sm:text-xs text-red-500 mt-1.5 ml-1 animate-in slide-in-from-top-1 duration-200">Passwords do not match</p>
                    )}
                  </div>
              ...
              {/* Terms - Outside Card */}
              <p className="text-[11px] sm:text-xs text-center text-[#9CA3AF] mt-5 sm:mt-6 px-4">
                By creating an account, you agree to our{' '}
                <a href="#" className="text-[#FF6636] hover:underline transition-colors duration-200">Terms of Service</a>
                {' '}and{' '}
                <a href="#" className="text-[#FF6636] hover:underline transition-colors duration-200">Privacy Policy</a>
              </p>
            </div>
          </div>
        </div>
